...
// WORKAROUND: Cardano and Midnight standards ship with their protocol
// changesets (TokenStandard.CardanoHyp* / MidnightHyp*); until this app's SDK
// version carries them, recognize them the same way as the Starknet ones above.
...
const MIDNIGHT_COLLATERAL_STANDARDS = ['MidnightHypNative', 'MidnightHypCollateral'];
export function isCollateralRoute(tokenStandard?: string): boolean {
  ...
  // WORKAROUND: Also check for Starknet, Cardano and Midnight collateral standards
  return (
    STARKNET_COLLATERAL_STANDARDS.includes(tokenStandard) ||
    CARDANO_COLLATERAL_STANDARDS.includes(tokenStandard) ||
    MIDNIGHT_COLLATERAL_STANDARDS.includes(tokenStandard)
  );
}
